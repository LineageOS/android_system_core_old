...
static int format_ext4(char *fs_blkdev, char *fs_mnt_point, uint64_t dev_sz, uint64_t footer, bool crypt_footer)
{
    int rc = 0;
    int status;
...
static int format_f2fs(char *fs_blkdev, uint64_t dev_sz, uint64_t footer, bool crypt_footer)
{
    int status;
...
int fs_mgr_do_format(struct fstab_rec *fstab, bool crypt_footer)
{
    ...
    if (!strncmp(fstab->fs_type, "f2fs", 4)) {
        ...
        rc = format_ext4(fstab->blk_device, fstab->mount_point, fstab->length, fstab->length, crypt_footer);
    } else {
