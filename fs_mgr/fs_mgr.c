...
int fs_mgr_mount_all(struct fstab *fstab)
{
    ...
    for (i = 0; i < fstab->num_entries; i++) {
        ...
        int compare_len = strlen(detected_fs_type);
        /* ext formats are mutually compatible by design */
        if (strncmp(detected_fs_type,"ext",3) && compare_len == 4) compare_len = 3;
        if (detected_fs_type && strncmp(fstab->recs[i].fs_type, detected_fs_type, compare_len) != 0) {
            continue;
        }
