 ...
 * What is this?
 * 
 * storage is a program that uses FUSE to emulate FAT-on-sdcard style
 * directory permissions (all files are given fixed owner, group, and 
 * permissions at creation, owner, group, and permissions are not 
 * changeable, symlinks and hardlinks are not createable, etc.
 *
 * usage:  storage <source> <destination> <uid> <gid>
 *
 * It must be run as root, but will change to uid/gid as soon as it
 * mounts a filesystem on <destination>.  It will refuse to run if uid or
 * gid are zero.
 *
 *
...
int main(int argc, char **argv)
{
    ...
    res = mount("/dev/fuse", dst, "fuse", MS_NOSUID | MS_NODEV, opts);
    if (res < 0) {
