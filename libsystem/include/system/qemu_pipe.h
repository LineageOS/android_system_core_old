...
static __inline__ int qemu_pipe_open(const char* pipeName) {
    ...
    if (ret != (ssize_t)pipeNameLen + 1) {
        QEMU_PIPE_DEBUG("%s: Could not connect to %s pipe service: %s", __FUNCTION__, pipeName,
                        strerror(errno));
        if (ret == 0) {
            errno = ECONNRESET;
        } else if (ret > 0) {
...
static int __inline__ qemu_pipe_frame_send(int fd, const void* buff, size_t len) {
    char header[5];
